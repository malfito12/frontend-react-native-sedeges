export const relacionesEspaciales = [
    {
        title: 'TEST 1',
        id: 1,
        factor: 'RELACIONES ESPACIALES',
        description: 'grafico',
        contenido: {
            instructions: 'A continuacion hay varias figuras; unas corresponden a la derecha y otros a la izquierda: Lo que usted tiene que hacer es decidir en cada caso si es derecha(D) o izquierda(I).',
            ejm: {
                preguntas: {
                    a: null,
                    b: null
                },
                resp: { respuesta: require('../images/RelacionesEspaciales/ejm1.png') },

            },
            preguntas: [
                {
                    pregunta: {
                        pre_id: 0,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/1.png'), puntaje: 'I' },
                    }
                },
                {
                    pregunta: {
                        pre_id: 1,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/2.png'), puntaje: 'D' },
                    }
                },
                {
                    pregunta: {
                        pre_id: 2,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/3.png'), puntaje: 'D' },
                    }
                },
                {
                    pregunta: {
                        pre_id: 3,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/4.png'), puntaje: 'I' },
                    }
                },
                {
                    pregunta: {
                        pre_id: 4,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/5.png'), puntaje: 'D' },
                    }
                },
                {
                    pregunta: {
                        pre_id: 5,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/6.png'), puntaje: 'D' },
                    }
                },
                {
                    pregunta: {
                        pre_id: 6,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/7.png'), puntaje: 'I' },
                    }
                },
                {
                    pregunta: {
                        pre_id: 7,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/8.png'), puntaje: 'D' },
                    }
                },
                {
                    pregunta: {
                        pre_id: 8,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/9.png'), puntaje: 'I' },
                    }
                },
                {
                    pregunta: {
                        pre_id: 9,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/10.png'), puntaje: 'D' },
                    }
                },
                {
                    pregunta: {
                        pre_id: 10,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/11.png'), puntaje: 'I' },
                    }
                },
                {
                    pregunta: {
                        pre_id: 11,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/12.png'), puntaje: 'I' },
                    }
                },
                {
                    pregunta: {
                        pre_id: 12,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/13.png'), puntaje: 'D' },
                    }
                },
                {
                    pregunta: {
                        pre_id: 13,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/14.png'), puntaje: 'D' },
                    }
                },
                {
                    pregunta: {
                        pre_id: 14,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/15.png'), puntaje: 'D' },
                    }
                },
                {
                    pregunta: {
                        pre_id: 15,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/16.png'), puntaje: 'D' },
                    }
                },
                {
                    pregunta: {
                        pre_id: 16,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/17.png'), puntaje: 'D' },
                    }
                },
                {
                    pregunta: {
                        pre_id: 17,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/18.png'), puntaje: 'I' },
                    }
                },
                {
                    pregunta: {
                        pre_id: 18,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/19.png'), puntaje: 'D' },
                    }
                },
                {
                    pregunta: {
                        pre_id: 19,
                        preguntas: 'Marque si la imagen es Derecha o Izquierda',
                        resp: { respuesta: require('../images/RelacionesEspaciales/20.png'), puntaje: 'I' },
                    }
                },
            ]
        }
    },
    {
        title: 'TEST 2',
        id: 2,
        factor: 'RELACIONES ESPACIALES',
        description: 'grafico',
        contenido: {
            instructions: 'En cada una de series de figura que siguen, usted deberá encontrar la que sea igual a la primera aunque esté en otra posicion.',
            ejm: {
                preguntas: {
                    a: null,
                    b: null
                },
                resp: [
                    { respuesta: require('../images/RelacionesEspaciales2/ejm0.png') },
                    { respuesta: require('../images/RelacionesEspaciales2/ejm1.png') },
                    { respuesta: require('../images/RelacionesEspaciales2/ejm2.png') },
                    { respuesta: require('../images/RelacionesEspaciales2/ejm3.png') },
                    { respuesta: require('../images/RelacionesEspaciales2/ejm4.png') },
                ]
            },
            preguntas: [
                {
                    pregunta: {
                        pre_id: 0,
                        preguntas: 'Marque la figura que sea igual a la primera aunque este en otra posición',
                        resp: [
                            { respuesta: require('../images/RelacionesEspaciales2/p1-0.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p1-1.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p1-2.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p1-3.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p1-4.png') },
                        ],
                        puntaje: '3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 1,
                        preguntas: 'Marque la figura que sea igual a la primera aunque este en otra posición',
                        resp: [
                            { respuesta: require('../images/RelacionesEspaciales2/p2-0.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p2-1.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p2-2.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p2-3.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p2-4.png') },
                        ],
                        puntaje: '1',
                    }
                },
                {
                    pregunta: {
                        pre_id: 2,
                        preguntas: 'Marque la figura que sea igual a la primera aunque este en otra posición',
                        resp: [
                            { respuesta: require('../images/RelacionesEspaciales2/p3-0.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p3-1.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p3-2.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p3-3.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p3-4.png') },
                        ],
                        puntaje: '4',
                    }
                },
                {
                    pregunta: {
                        pre_id: 3,
                        preguntas: 'Marque la figura que sea igual a la primera aunque este en otra posición',
                        resp: [
                            { respuesta: require('../images/RelacionesEspaciales2/p4-0.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p4-1.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p4-2.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p4-3.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p4-4.png') },
                        ],
                        puntaje: '3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 4,
                        preguntas: 'Marque la figura que sea igual a la primera aunque este en otra posición',
                        resp: [
                            { respuesta: require('../images/RelacionesEspaciales2/p5-0.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p5-1.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p5-2.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p5-3.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p5-4.png') },
                        ],
                        puntaje: '3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 5,
                        preguntas: 'Marque la figura que sea igual a la primera aunque este en otra posición',
                        resp: [
                            { respuesta: require('../images/RelacionesEspaciales2/p6-0.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p6-1.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p6-2.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p6-3.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p6-4.png') },
                        ],
                        puntaje: '3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 6,
                        preguntas: 'Marque la figura que sea igual a la primera aunque este en otra posición',
                        resp: [
                            { respuesta: require('../images/RelacionesEspaciales2/p7-0.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p7-1.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p7-2.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p7-3.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p7-4.png') },
                        ],
                        puntaje: '1',
                    }
                },
                {
                    pregunta: {
                        pre_id: 7,
                        preguntas: 'Marque la figura que sea igual a la primera aunque este en otra posición',
                        resp: [
                            { respuesta: require('../images/RelacionesEspaciales2/p8-0.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p8-1.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p8-2.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p8-3.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p8-4.png') },
                        ],
                        puntaje: '4',
                    }
                },
                {
                    pregunta: {
                        pre_id: 8,
                        preguntas: 'Marque la figura que sea igual a la primera aunque este en otra posición',
                        resp: [
                            { respuesta: require('../images/RelacionesEspaciales2/p9-0.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p9-1.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p9-2.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p9-3.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p9-4.png') },
                        ],
                        puntaje: '3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 9,
                        preguntas: 'Marque la figura que sea igual a la primera aunque este en otra posición',
                        resp: [
                            { respuesta: require('../images/RelacionesEspaciales2/p10-0.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p10-1.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p10-2.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p10-3.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p10-4.png') },
                        ],
                        puntaje: '4',
                    }
                },
                {
                    pregunta: {
                        pre_id: 10,
                        preguntas: 'Marque la figura que sea igual a la primera aunque este en otra posición',
                        resp: [
                            { respuesta: require('../images/RelacionesEspaciales2/p11-0.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p11-1.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p11-2.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p11-3.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p11-4.png') },
                        ],
                        puntaje: '2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 11,
                        preguntas: 'Marque la figura que sea igual a la primera aunque este en otra posición',
                        resp: [
                            { respuesta: require('../images/RelacionesEspaciales2/p12-0.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p12-1.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p12-2.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p12-3.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p12-4.png') },
                        ],
                        puntaje: '4',
                    }
                },
                {
                    pregunta: {
                        pre_id: 12,
                        preguntas: 'Marque la figura que sea igual a la primera aunque este en otra posición',
                        resp: [
                            { respuesta: require('../images/RelacionesEspaciales2/p13-0.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p13-1.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p13-2.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p13-3.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p13-4.png') },
                        ],
                        puntaje: '2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 13,
                        preguntas: 'Marque la figura que sea igual a la primera aunque este en otra posición',
                        resp: [
                            { respuesta: require('../images/RelacionesEspaciales2/p14-0.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p14-1.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p14-2.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p14-3.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p14-4.png') },
                        ],
                        puntaje: '3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 14,
                        preguntas: 'Marque la figura que sea igual a la primera aunque este en otra posición',
                        resp: [
                            { respuesta: require('../images/RelacionesEspaciales2/p15-0.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p15-1.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p15-2.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p15-3.png') },
                            { respuesta: require('../images/RelacionesEspaciales2/p15-4.png') },
                        ],
                        puntaje: '4',
                    }
                },
            ]
        }
    }
]

export const razonamientoLogico = [
    {
        title: 'TEST 3',
        id: 3,
        factor: 'RAZONAMIENTO LOGICO',
        description: 'grafico',
        contenido: {
            instructions: 'Las tres primeras figuras de cada fila guardan alguna semejansa. Busque entre las cuatro que siguen la que mas se asemeje a las tres primeras.',
            ejm: {
                preguntas: {
                    a: null,
                    b: null
                },
                resp: [
                    { respuesta: require('../images/RazonamientoLogico/ejm0.png') },
                    { respuesta: require('../images/RazonamientoLogico/ejm1.png') },
                    { respuesta: require('../images/RazonamientoLogico/ejm2.png') },
                    { respuesta: require('../images/RazonamientoLogico/ejm3.png') },
                    { respuesta: require('../images/RazonamientoLogico/ejm4.png') },
                ]
            },
            preguntas: [
                {
                    pregunta: {
                        pre_id: 0,
                        preguntas: 'Busque entre las cuatro figuras que siguen la que mas se asemeje a las tres primeras',
                        resp: [
                            { respuesta: require('../images/RazonamientoLogico/p1-0.png') },
                            { respuesta: require('../images/RazonamientoLogico/p1-1.png') },
                            { respuesta: require('../images/RazonamientoLogico/p1-2.png') },
                            { respuesta: require('../images/RazonamientoLogico/p1-3.png') },
                            { respuesta: require('../images/RazonamientoLogico/p1-4.png') },
                        ],
                        puntaje:'3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 1,
                        preguntas: 'Busque entre las cuatro figuras que siguen la que mas se asemeje a las tres primeras',
                        resp: [
                            { respuesta: require('../images/RazonamientoLogico/p2-0.png') },
                            { respuesta: require('../images/RazonamientoLogico/p2-1.png') },
                            { respuesta: require('../images/RazonamientoLogico/p2-2.png') },
                            { respuesta: require('../images/RazonamientoLogico/p2-3.png') },
                            { respuesta: require('../images/RazonamientoLogico/p2-4.png') },
                        ],
                        puntaje:'1',
                    }
                },
                {
                    pregunta: {
                        pre_id: 2,
                        preguntas: 'Busque entre las cuatro figuras que siguen la que mas se asemeje a las tres primeras',
                        resp: [
                            { respuesta: require('../images/RazonamientoLogico/p3-0.png') },
                            { respuesta: require('../images/RazonamientoLogico/p3-1.png') },
                            { respuesta: require('../images/RazonamientoLogico/p3-2.png') },
                            { respuesta: require('../images/RazonamientoLogico/p3-3.png') },
                            { respuesta: require('../images/RazonamientoLogico/p3-4.png') },
                        ],
                        puntaje:'4',
                    }
                },
                {
                    pregunta: {
                        pre_id: 3,
                        preguntas: 'Busque entre las cuatro figuras que siguen la que mas se asemeje a las tres primeras',
                        resp: [
                            { respuesta: require('../images/RazonamientoLogico/p4-0.png') },
                            { respuesta: require('../images/RazonamientoLogico/p4-1.png') },
                            { respuesta: require('../images/RazonamientoLogico/p4-2.png') },
                            { respuesta: require('../images/RazonamientoLogico/p4-3.png') },
                            { respuesta: require('../images/RazonamientoLogico/p4-4.png') },
                        ],
                        puntaje:'3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 4,
                        preguntas: 'Busque entre las cuatro figuras que siguen la que mas se asemeje a las tres primeras',
                        resp: [
                            { respuesta: require('../images/RazonamientoLogico/p5-0.png') },
                            { respuesta: require('../images/RazonamientoLogico/p5-1.png') },
                            { respuesta: require('../images/RazonamientoLogico/p5-2.png') },
                            { respuesta: require('../images/RazonamientoLogico/p5-3.png') },
                            { respuesta: require('../images/RazonamientoLogico/p5-4.png') },
                        ],
                        puntaje:'3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 5,
                        preguntas: 'Busque entre las cuatro figuras que siguen la que mas se asemeje a las tres primeras',
                        resp: [
                            { respuesta: require('../images/RazonamientoLogico/p6-0.png') },
                            { respuesta: require('../images/RazonamientoLogico/p6-1.png') },
                            { respuesta: require('../images/RazonamientoLogico/p6-2.png') },
                            { respuesta: require('../images/RazonamientoLogico/p6-3.png') },
                            { respuesta: require('../images/RazonamientoLogico/p6-4.png') },
                        ],
                        puntaje:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 6,
                        preguntas: 'Busque entre las cuatro figuras que siguen la que mas se asemeje a las tres primeras',
                        resp: [
                            { respuesta: require('../images/RazonamientoLogico/p7-0.png') },
                            { respuesta: require('../images/RazonamientoLogico/p7-1.png') },
                            { respuesta: require('../images/RazonamientoLogico/p7-2.png') },
                            { respuesta: require('../images/RazonamientoLogico/p7-3.png') },
                            { respuesta: require('../images/RazonamientoLogico/p7-4.png') },
                        ],
                        puntaje:'3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 7,
                        preguntas: 'Busque entre las cuatro figuras que siguen la que mas se asemeje a las tres primeras',
                        resp: [
                            { respuesta: require('../images/RazonamientoLogico/p8-0.png') },
                            { respuesta: require('../images/RazonamientoLogico/p8-1.png') },
                            { respuesta: require('../images/RazonamientoLogico/p8-2.png') },
                            { respuesta: require('../images/RazonamientoLogico/p8-3.png') },
                            { respuesta: require('../images/RazonamientoLogico/p8-4.png') },
                        ],
                        puntaje:'3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 8,
                        preguntas: 'Busque entre las cuatro figuras que siguen la que mas se asemeje a las tres primeras',
                        resp: [
                            { respuesta: require('../images/RazonamientoLogico/p9-0.png') },
                            { respuesta: require('../images/RazonamientoLogico/p9-1.png') },
                            { respuesta: require('../images/RazonamientoLogico/p9-2.png') },
                            { respuesta: require('../images/RazonamientoLogico/p9-3.png') },
                            { respuesta: require('../images/RazonamientoLogico/p9-4.png') },
                        ],
                        puntaje:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 9,
                        preguntas: 'Busque entre las cuatro figuras que siguen la que mas se asemeje a las tres primeras',
                        resp: [
                            { respuesta: require('../images/RazonamientoLogico/p10-0.png') },
                            { respuesta: require('../images/RazonamientoLogico/p10-1.png') },
                            { respuesta: require('../images/RazonamientoLogico/p10-2.png') },
                            { respuesta: require('../images/RazonamientoLogico/p10-3.png') },
                            { respuesta: require('../images/RazonamientoLogico/p10-4.png') },
                        ],
                        puntaje:'3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 10,
                        preguntas: 'Busque entre las cuatro figuras que siguen la que mas se asemeje a las tres primeras',
                        resp: [
                            { respuesta: require('../images/RazonamientoLogico/p11-0.png') },
                            { respuesta: require('../images/RazonamientoLogico/p11-1.png') },
                            { respuesta: require('../images/RazonamientoLogico/p11-2.png') },
                            { respuesta: require('../images/RazonamientoLogico/p11-3.png') },
                            { respuesta: require('../images/RazonamientoLogico/p11-4.png') },
                        ],
                        puntaje:'4',
                    }
                },
                {
                    pregunta: {
                        pre_id: 11,
                        preguntas: 'Busque entre las cuatro figuras que siguen la que mas se asemeje a las tres primeras',
                        resp: [
                            { respuesta: require('../images/RazonamientoLogico/p12-0.png') },
                            { respuesta: require('../images/RazonamientoLogico/p12-1.png') },
                            { respuesta: require('../images/RazonamientoLogico/p12-2.png') },
                            { respuesta: require('../images/RazonamientoLogico/p12-3.png') },
                            { respuesta: require('../images/RazonamientoLogico/p12-4.png') },
                        ],
                        puntaje:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 12,
                        preguntas: 'Busque entre las cuatro figuras que siguen la que mas se asemeje a las tres primeras',
                        resp: [
                            { respuesta: require('../images/RazonamientoLogico/p13-0.png') },
                            { respuesta: require('../images/RazonamientoLogico/p13-1.png') },
                            { respuesta: require('../images/RazonamientoLogico/p13-2.png') },
                            { respuesta: require('../images/RazonamientoLogico/p13-3.png') },
                            { respuesta: require('../images/RazonamientoLogico/p13-4.png') },
                        ],
                        puntaje:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 13,
                        preguntas: 'Busque entre las cuatro figuras que siguen la que mas se asemeje a las tres primeras',
                        resp: [
                            { respuesta: require('../images/RazonamientoLogico/p14-0.png') },
                            { respuesta: require('../images/RazonamientoLogico/p14-1.png') },
                            { respuesta: require('../images/RazonamientoLogico/p14-2.png') },
                            { respuesta: require('../images/RazonamientoLogico/p14-3.png') },
                            { respuesta: require('../images/RazonamientoLogico/p14-4.png') },
                        ],
                        puntaje:'3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 14,
                        preguntas: 'Busque entre las cuatro figuras que siguen la que mas se asemeje a las tres primeras',
                        resp: [
                            { respuesta: require('../images/RazonamientoLogico/p15-0.png') },
                            { respuesta: require('../images/RazonamientoLogico/p15-1.png') },
                            { respuesta: require('../images/RazonamientoLogico/p15-2.png') },
                            { respuesta: require('../images/RazonamientoLogico/p15-3.png') },
                            { respuesta: require('../images/RazonamientoLogico/p15-4.png') },
                        ],
                        puntaje:'2',
                    }
                },
            ]
        }
    },
    {
        title: 'TEST 4',
        id: 4,
        factor: 'RAZONAMIENTO LOGICO',
        description: 'logico',
        contenido: {
            instructions: 'Lee antentamente cada grupo de oraciones escritas con letras mayusculas, asi como las tres posibles respuestas; luego escoja la mas adecuada.',
            ejm: {
                preguntas: {
                    a: 'TODOS LOS SERES QUE TIENE CUATRO PATARS SON ANIMALES',
                    b: 'TODOS LOS CABALLEROS TIENE CUATRO PATAS',
                },
                resp: [
                    { respuesta: 'Otros seres como los caballeros pueden caminar' },
                    { respuesta: 'Todos los caballeros peden caminar' },
                    { respuesta: 'Todos los caballeros son animales' }
                ]
            },
            preguntas: [
                {
                    pregunta: {
                        pre_id: 0,
                        preguntas: {
                            a: 'EL SEÑOR "X" ES UN AVIADOR',
                            b: 'EL SEÑOR "X" ES UN JEFE DE SCOUTS',
                            c: 'Luego:',
                        },
                        resp: [
                            { respuesta: 'Los aviadores son buenos jefes de scouts', puntaje: 5 },
                            { respuesta: 'Un aviador puede ser jefe de scouts', puntaje: 10 },
                            { respuesta: 'Los jefes de scouts son buenos aviadores', puntaje: 15 },
                        ],
                        respCorrecta:'B',
                    }
                },
                {
                    pregunta: {
                        pre_id: 1,
                        preguntas: {
                            a: 'TRES MUCHACHOS ESTAN ESCALANDO UN CERRO, LUIS SE ENCUENTRA MAS ARRIBA QUE ENRIQUE, FRANCISCO SE ENCUENTRA MAS ARRIBA QUE LUIS.',
                            b: null,
                            c: '¿Cual de los muchados se encuentra en el segundo lugar?',
                        },
                        resp: [
                            { respuesta: 'Luis', puntaje: 5 },
                            { respuesta: 'Enrique', puntaje: 10 },
                            { respuesta: 'Fancisco', puntaje: 15 },
                        ],
                        respCorrecta:'A',
                    }
                },
                {
                    pregunta: {
                        pre_id: 2,
                        preguntas: {
                            a: 'NINGUN SER HOMANO ESTA LIBRE DE ACCIONES',
                            b: 'LOS CHOFERES SON SERES HUMANOS',
                            c: 'Luego:',
                        },
                        resp: [
                            { respuesta: 'La vida de ningun ser humano esta asegurada', puntaje: 5 },
                            { respuesta: 'Los chóferes de vehiculos no estan libres de cometer accidentes', puntaje: 10 },
                            { respuesta: 'Pocos chóferes conducen con seguridad sus vehiculos', puntaje: 15 },
                        ],
                        respCorrecta:'B',
                    }
                },
                {
                    pregunta: {
                        pre_id: 3,
                        preguntas: {
                            a: 'SI EL PERMANECE CON SUS AMIGOS SUFRIRÁ UN PERJUICIO, Y SI EL DIJA A SUS AMIGOS TAMBIEN SUFRIRÁ UN PERJUICIO, PERO EL DEBE PERMANECER CON SUS AMIGOS O DEJARLOS',
                            b: null,
                            c: 'Luego:',
                        },
                        resp: [
                            { respuesta: 'El debe permanecer con sus amigos', puntaje: 5 },
                            { respuesta: 'El debe hacerse el fuerte para dejar a sus amigos', puntaje: 10 },
                            { respuesta: 'El sufrirá un perjuicio', puntaje: 15 },
                        ],
                        respCorrecta:'C',
                    }
                },
                {
                    pregunta: {
                        pre_id: 4,
                        preguntas: {
                            a: 'TODOS LOS CUADRADOS TIENEN CUATRO LADOS IGUALES.',
                            b: 'ESTA FIGURA NO TIENE CUATRO LADOS IGUALES',
                            c: 'Luego:',
                        },
                        resp: [
                            { respuesta: 'Es un circulo', puntaje: 5 },
                            { respuesta: 'No es un cuadrado', puntaje: 10 },
                            { respuesta: 'Es un triangulo', puntaje: 15 },
                        ],
                        respCorrecta:'B',
                    }
                },
                {
                    pregunta: {
                        pre_id: 5,
                        preguntas: {
                            a: 'UNA PERSONA PUEDE SER EXTRANJERA, O NATURAL DEL PAIS',
                            b: 'ESTA PERSONAL NO ES EXTRANJERA',
                            c: 'Luego:',
                        },
                        resp: [
                            { respuesta: 'Es nacionalizada', puntaje: 5 },
                            { respuesta: 'Es natural de pais', puntaje: 10 },
                            { respuesta: 'Es un turista', puntaje: 15 },
                        ],
                        respCorrecta:'B',
                    }
                },
                {
                    pregunta: {
                        pre_id: 6,
                        preguntas: {
                            a: 'LA CALLE "S" ES PARALELA A LA CALLE "D"',
                            b: 'LA CALLE "D" ES PARALELA A LA CALLE "C"',
                            c: 'Luego:',
                        },
                        resp: [
                            { respuesta: 'La calle "S" esta al este de "D"', puntaje: 5 },
                            { respuesta: 'La calle "C" cruza a "S"', puntaje: 10 },
                            { respuesta: 'La calle "S" es paralera "C"', puntaje: 15 },
                        ],
                        respCorrecta:'C',
                    }
                },
                {
                    pregunta: {
                        pre_id: 7,
                        preguntas: {
                            a: 'SUS HERMANOS PUEDE SER MAS INTELIGENTES QUE UD., TAN INTELIGENTE COMO UD. O MENOS INTELIGENTE QUE UD., PERO ESTE HERMANO NO ES MAS INTELIGENTE QUE UD., NU TAMPOCO ES MENOS INTELIGENTE QUE UD.',
                            b: null,
                            c: 'Luego:',
                        },
                        resp: [
                            { respuesta: 'Su hermnao es menos inteligente que Ud.', puntaje: 5 },
                            { respuesta: 'Su hermano es tan inteligente como Ud.', puntaje: 10 },
                            { respuesta: 'Su hermando es mas inteligente que Ud.', puntaje: 15 },
                        ],
                        respCorrecta:'B',
                    }
                },
                {
                    pregunta: {
                        pre_id: 8,
                        preguntas: {
                            a: 'EDUARDO ES MEJOR JUGADOR DE FUTBOL QUE JAIME, JAIME ES TAN JUGADOR COMO GUILLERMO',
                            b: null,
                            c: '¿Quien es el mejor jugador de futbol?',
                        },
                        resp: [
                            { respuesta: 'Eduardo', puntaje: 5 },
                            { respuesta: 'Guillermo', puntaje: 10 },
                            { respuesta: 'Jaime', puntaje: 15 },
                        ],
                        respCorrecta:'A',
                    }
                },
                {
                    pregunta: {
                        pre_id: 9,
                        preguntas: {
                            a: '"A" PESA MENOS QUE "B"',
                            b: '"B" PESA MENOS QUE "C"',
                            c: 'Luego:',
                        },
                        resp: [
                            { respuesta: '"B" pesa menos que "C"', puntaje: 5 },
                            { respuesta: '"A" pesa igual que "B"', puntaje: 10 },
                            { respuesta: '"A" pesa menos que "C"', puntaje: 15 },
                        ],
                        respCorrecta:'C',
                    }
                },
                {
                    pregunta: {
                        pre_id: 10,
                        preguntas: {
                            a: 'UNA CAJA PUEDE CONTENER ORO, PLATA O DIAMANTE',
                            b: 'ESTA CAJA NO CONTIENE PLATA',
                            c: 'Luego esta caja',
                        },
                        resp: [
                            { respuesta: 'Contiene diamantes', puntaje: 5 },
                            { respuesta: 'Contiene oro o diamantes', puntaje: 10 },
                            { respuesta: 'No se sabe que contiene', puntaje: 15 },
                        ],
                        respCorrecta:'B',
                    }
                },
                {
                    pregunta: {
                        pre_id: 11,
                        preguntas: {
                            a: 'SI ÉL FORMA PARTE DEL EQUIPO DEBE EVITAR REÑIR CON EL CAPITAN Y CON EL ENTRENADOR PERO AUNQUE NO LO QUIERA, NO PODRÁ EVITAR LAS RIÑAS CON EL CAPITAN Y CON EL ENTRENADOR',
                            b: null,
                            c: 'Por consiguiente:',
                        },
                        resp: [
                            { respuesta: 'El no debe formar parte de equipo', puntaje: 5 },
                            { respuesta: 'El puede perder el apoyo de sus compañeros', puntaje: 10 },
                            { respuesta: 'El puede presentar razonablmente una queja', puntaje: 15 },
                        ],
                        respCorrecta:'A',
                    }
                },
                {
                    pregunta: {
                        pre_id: 12,
                        preguntas: {
                            a: 'CUANDO UNA QUEJA ES INJUSTA RESULTA DESATINADO RECHAZAR SU DISCUSION POR LA JUNTA DE PROFESORES, SI LA QUEJA ES IN JUSTA SU RECHAZO ES IMPERSONABLE, PERO UNA QUEJA PUEDE SER JUSTA O INJUSTA',
                            b: null,
                            c: 'Entonces:',
                        },
                        resp: [
                            { respuesta: 'El rechazo de la queja es justificable', puntaje: 5 },
                            { respuesta: 'El rechazo se discute sin reserva', puntaje: 10 },
                            { respuesta: 'El reacho de la queja es desatinado, o es imperdonable', puntaje: 15 },
                        ],
                        respCorrecta:'C',
                    }
                },
                {
                    pregunta: {
                        pre_id: 13,
                        preguntas: {
                            a: 'LA CASA "A" ESTA SITUADA AL NOR-ESTE DE LA CASA "B"',
                            b: 'LA CASA "B" ESTA SITUADA AL NOR-ESTE DE LA CASA "C"',
                            c: 'Luego:',
                        },
                        resp: [
                            { respuesta: 'La casa "A" queda cerca de la casa de "C"', puntaje: 5 },
                            { respuesta: 'La casa "C" está más cercana a la casa "A" que a la casa "B"', puntaje: 10 },
                            { respuesta: 'La casa "A" queda al nor-este de la casa "C"', puntaje: 15 },
                        ],
                        respCorrecta:'C',
                    }
                },
                {
                    pregunta: {
                        pre_id: 14,
                        preguntas: {
                            a: '"W" ESTÁ ENTRE "X" y "Y"',
                            b: '"X" ESTÁ ENTRE "Y" y "Z"',
                            c: 'Luego:',
                        },
                        resp: [
                            { respuesta: '"W" no está entre "Y" y "Z"', puntaje: 5 },
                            { respuesta: '"W" está entre "X" y "Z"', puntaje: 10 },
                            { respuesta: '"W" está más cerca de "X" que "Z"', puntaje: 15 },
                        ],
                        respCorrecta:'C',
                    }
                },
            ]
        }
    },
]

export const razonamientoNumerico = [
    {
        title: 'TEST 5 - PRIMERA PARTE',
        id: 5,
        factor: 'RAZONAMIENTO NUMERICO',
        description: 'logico',
        contenido: {
            instructions: 'En cada serie de nuemros, hay uno que es incorrecto. Busque ese número',
            ejm: {
                preguntas: '2 4 6 8 9 10 12 14',
                resp: '9'
            },
            preguntas: [
                {
                    pregunta: {
                        pre_id: 1,
                        preguntas: 'Busque el Numero que Sobre',
                        resp: [
                            { respuesta: '14', puntaje: 2 },
                            { respuesta: '12', puntaje: 4 },
                            { respuesta: '10', puntaje: 6 },
                            { respuesta: '8', puntaje: 8 },
                            { respuesta: '7', puntaje: 10 },
                            { respuesta: '6', puntaje: 12 },
                            { respuesta: '4', puntaje: 14 },
                        ],
                        respCorrecta:'7',
                    }
                },
                {
                    pregunta: {
                        pre_id: 2,
                        preguntas: 'Busque el Numero que Sobre',
                        resp: [
                            { respuesta: '19', puntaje: 2 },
                            { respuesta: '16', puntaje: 4 },
                            { respuesta: '13', puntaje: 6 },
                            { respuesta: '11', puntaje: 8 },
                            { respuesta: '10', puntaje: 10 },
                            { respuesta: '7', puntaje: 12 },
                            { respuesta: '4', puntaje: 14 },
                        ],
                        respCorrecta:'11',
                    }
                },
                {
                    pregunta: {
                        pre_id: 3,
                        preguntas: 'Busque el Numero que Sobre',
                        resp: [
                            { respuesta: '1', puntaje: 2 },
                            { respuesta: '5', puntaje: 4 },
                            { respuesta: '9', puntaje: 6 },
                            { respuesta: '13', puntaje: 8 },
                            { respuesta: '15', puntaje: 10 },
                            { respuesta: '17', puntaje: 12 },
                        ],
                        respCorrecta:'15',
                    }
                },
                {
                    pregunta: {
                        pre_id: 4,
                        preguntas: 'Busque el Numero que Sobre',
                        resp: [
                            { respuesta: '4', puntaje: 2 },
                            { respuesta: '5', puntaje: 4 },
                            { respuesta: '7', puntaje: 6 },
                            { respuesta: '8', puntaje: 8 },
                            { respuesta: '10', puntaje: 10 },
                            { respuesta: '11', puntaje: 12 },
                            { respuesta: '12', puntaje: 14 },
                            { respuesta: '13', puntaje: 16 },
                        ],
                        respCorrecta:'12',
                    }
                },
                {
                    pregunta: {
                        pre_id: 5,
                        preguntas: 'Busque el Numero que Sobre',
                        resp: [
                            { respuesta: '2', puntaje: 2 },
                            { respuesta: '4', puntaje: 4 },
                            { respuesta: '5', puntaje: 6 },
                            { respuesta: '7', puntaje: 8 },
                            { respuesta: '8', puntaje: 10 },
                            { respuesta: '9', puntaje: 12 },
                            { respuesta: '10', puntaje: 14 },
                            { respuesta: '11', puntaje: 16 },
                            { respuesta: '13', puntaje: 18 },
                            { respuesta: '14', puntaje: 20 },
                        ],
                        respCorrecta:'9',
                    }
                },
                {
                    pregunta: {
                        pre_id: 6,
                        preguntas: 'Busque el Numero que Sobre',
                        resp: [
                            { respuesta: '0', puntaje: 2 },
                            { respuesta: '7', puntaje: 4 },
                            { respuesta: '14', puntaje: 6 },
                            { respuesta: '19', puntaje: 8 },
                            { respuesta: '24', puntaje: 10 },
                            { respuesta: '27', puntaje: 12 },
                            { respuesta: '29', puntaje: 14 },
                            { respuesta: '30', puntaje: 16 },
                            { respuesta: '31', puntaje: 18 },
                        ],
                        respCorrecta:'29',
                    }
                },
                {
                    pregunta: {
                        pre_id: 7,
                        preguntas: 'Busque el Numero que Sobre',
                        resp: [
                            { respuesta: '20', puntaje: 2 },
                            { respuesta: '17', puntaje: 4 },
                            { respuesta: '15', puntaje: 6 },
                            { respuesta: '14', puntaje: 8 },
                            { respuesta: '11', puntaje: 10 },
                            { respuesta: '9', puntaje: 12 },
                            { respuesta: '8', puntaje: 14 },
                            { respuesta: '7', puntaje: 16 },
                            { respuesta: '5', puntaje: 18 },
                            { respuesta: '3', puntaje: 20 },
                            { respuesta: '2', puntaje: 22 },
                        ],
                        respCorrecta:'7',
                    }
                },
                {
                    pregunta: {
                        pre_id: 8,
                        preguntas: 'Busque el Numero que Sobre',
                        resp: [
                            { respuesta: '21', puntaje: 2 },
                            { respuesta: '20', puntaje: 4 },
                            { respuesta: '18', puntaje: 6 },
                            { respuesta: '15', puntaje: 8 },
                            { respuesta: '14', puntaje: 10 },
                            { respuesta: '12', puntaje: 12 },
                            { respuesta: '10', puntaje: 14 },
                            { respuesta: '9', puntaje: 16 },
                            { respuesta: '8', puntaje: 18 },
                            { respuesta: '6', puntaje: 20 },
                            { respuesta: '3', puntaje: 22 },
                        ],
                        respCorrecta:'10',
                    }
                },
                {
                    pregunta: {
                        pre_id: 9,
                        preguntas: 'Busque el Numero que Sobre',
                        resp: [
                            { respuesta: '2', puntaje: 2 },
                            { respuesta: '3', puntaje: 4 },
                            { respuesta: '5', puntaje: 6 },
                            { respuesta: '8', puntaje: 8 },
                            { respuesta: '12', puntaje: 10 },
                            { respuesta: '17', puntaje: 12 },
                            { respuesta: '22', puntaje: 14 },
                            { respuesta: '23', puntaje: 16 },
                            { respuesta: '30', puntaje: 18 },
                        ],
                        respCorrecta:'22',
                    }
                },
                {
                    pregunta: {
                        pre_id: 10,
                        preguntas: 'Busque el Numero que Sobre',
                        resp: [
                            { respuesta: '20', puntaje: 2 },
                            { respuesta: '18', puntaje: 4 },
                            { respuesta: '19', puntaje: 6 },
                            { respuesta: '17', puntaje: 8 },
                            { respuesta: '18', puntaje: 10 },
                            { respuesta: '16', puntaje: 12 },
                            { respuesta: '17', puntaje: 14 },
                            { respuesta: '14', puntaje: 16 },
                            { respuesta: '15', puntaje: 18 },
                            { respuesta: '16', puntaje: 20 },
                        ],
                        respCorrecta:'14',
                    }
                },
            ]
        }
    },
    {
        title: 'TEST 5 - SEGUNDA PARTE',
        id: 6,
        factor: 'RAZONAMIENTO NUMERICO',
        description: 'logico',
        contenido: {
            instructions: 'Aqui hay varias series de numeros que aumentan o disminuyen. A las series les falta algunos nuemros para estar completas. Escoja entre las respuestas, el gtupo que contiene todos los nueros que falta.',
            ejm: {
                preguntas: {
                    a: '12  _   14  15  _   _   18',
                    b: 'Escoja el grupo correcto',
                },
                resp: [
                    { respuesta: '13, 15, 16' },
                    { respuesta: '13, 15, 17' },
                    { respuesta: '13, 16, 17' },
                    { respuesta: '14, 16, 17' },
                    { respuesta: '15, 16, 18' },
                ]
            },
            preguntas: [
                {
                    pregunta: {
                        pre_id: 1,
                        preguntas: [
                            { a: '1   4   _   10  _   _   19' },
                            { b: 'Busque los numeros que faltan' },
                        ],
                        resp: [
                            { respuesta: '5, 11, 18', puntaje: 2 },
                            { respuesta: '7, 13, 16', puntaje: 4 },
                            { respuesta: '5, 13, 16', puntaje: 6 },
                            { respuesta: '7, 11, 18', puntaje: 8 },
                            { respuesta: '5, 16, 18', puntaje: 10 },
                        ],
                        respCorrecta:'B',
                    }
                },
                {
                    pregunta: {
                        pre_id: 2,
                        preguntas: [
                            { a: '2   _   8   _   32  _' },
                            { b: 'Busque los numeros que faltan' },
                        ],
                        resp: [
                            { respuesta: '7, 13, 33', puntaje: 2 },
                            { respuesta: '4, 16, 37', puntaje: 4 },
                            { respuesta: '3, 15, 48', puntaje: 6 },
                            { respuesta: '4, 16, 64', puntaje: 8 },
                            { respuesta: '6, 24, 64', puntaje: 10 },
                        ],
                        respCorrecta:'D',
                    }
                },
                {
                    pregunta: {
                        pre_id: 3,
                        preguntas: [
                            { a: '44  37  _   _   16  _   2' },
                            { b: 'Busque los numeros que faltan' },
                        ],
                        resp: [
                            { respuesta: '30, 22, 8', puntaje: 2 },
                            { respuesta: '31, 22, 9', puntaje: 4 },
                            { respuesta: '30, 23, 9', puntaje: 6 },
                            { respuesta: '30, 21, 9', puntaje: 8 },
                            { respuesta: '31, 23, 8', puntaje: 10 },
                        ],
                        respCorrecta:'C',
                    }
                },
                {
                    pregunta: {
                        pre_id: 4,
                        preguntas: [
                            { a: '6   _   28  _   50  _   72' },
                            { b: 'Busque los numeros que faltan' },
                        ],
                        resp: [
                            { respuesta: '16, 38, 60', puntaje: 2 },
                            { respuesta: '16, 39, 61', puntaje: 4 },
                            { respuesta: '17, 38, 60', puntaje: 6 },
                            { respuesta: '11, 39, 61', puntaje: 8 },
                            { respuesta: '17, 39, 61', puntaje: 10 },
                        ],
                        respCorrecta:'E',
                    }
                },
                {
                    pregunta: {
                        pre_id: 5,
                        preguntas: [
                            { a: '83  70  _   44  _   _   5' },
                            { b: 'Busque los numeros que faltan' },
                        ],
                        resp: [
                            { respuesta: '57, 31, 18', puntaje: 2 },
                            { respuesta: '53, 33, 23', puntaje: 4 },
                            { respuesta: '57, 33, 19', puntaje: 6 },
                            { respuesta: '53, 31, 18', puntaje: 8 },
                            { respuesta: '57, 33, 19', puntaje: 10 },
                        ],
                        respCorrecta:'A',
                    }
                },
            ]
        }
    },
    {
        title: 'TEST 6',
        id: 7,
        factor: 'RAZONAMIENTO NUMERICO',
        description: 'logico',
        contenido: {
            instructions: 'A continuacion hay varios problemas y frente a cada uno, de ellos cuatro respuestas posibles precedidas por letras. Busque cual es la respuesta correcta.',
            ejm: {
                preguntas: 'Si tu tienes 5 Bs. y gastas 3 Bs. ¿ cuantos Bs. te quedan ?',
                resp: [
                    { respuesta: '1,00 Bs.' },
                    { respuesta: '2,00 Bs.' },
                    { respuesta: '3,00 Bs.' },
                    { respuesta: '5,00 Bs.' },
                ]
            },
            preguntas: [
                {
                    pregunta: {
                        pre_id: 0,
                        preguntas: [
                            { a: 'Un tren de carga viaja a una velocidad de 20 kilometros por hora.' },
                            { b: '¿Cunatos kilometros recorrerá en 4 horas?' },
                        ],
                        resp: [
                            { respuesta: '5', puntaje: 2 },
                            { respuesta: '24', puntaje: 4 },
                            { respuesta: '80', puntaje: 6 },
                            { respuesta: '60', puntaje: 8 },
                        ],
                        respCorrecta:'C',
                    }
                },
                {
                    pregunta: {
                        pre_id: 1,
                        preguntas: [
                            { a: '¿Cuantos metros de tela se podrá comprar con 150 Bs. a razón de 4 metros por 50 Bs.?' },
                            { b: null },
                        ],
                        resp: [
                            { respuesta: '9', puntaje: 2 },
                            { respuesta: '12', puntaje: 4 },
                            { respuesta: '15', puntaje: 6 },
                            { respuesta: '60', puntaje: 8 },
                        ],
                        respCorrecta:'B',
                    }
                },
                {
                    pregunta: {
                        pre_id: 2,
                        preguntas: [
                            { a: 'En un mapa cada media pulgada representa 20 millas.' },
                            { b: '¿Cuantas millas estaran representadas en 5 pulgadas?' },
                        ],
                        resp: [
                            { respuesta: '10', puntaje: 2 },
                            { respuesta: '20', puntaje: 4 },
                            { respuesta: '100', puntaje: 6 },
                            { respuesta: '200', puntaje: 8 },
                        ],
                        respCorrecta:'D',
                    }
                },
                {
                    pregunta: {
                        pre_id: 3,
                        preguntas: [
                            { a: 'Una Galleta cuesta 3 centavos, pero puede obtenerse una docena por 30 centavos.' },
                            { b: 'Cuantos economizaré si compro una docena' },
                        ],
                        resp: [
                            { respuesta: '10 ctvs.', puntaje: 2 },
                            { respuesta: '6 ctvs.', puntaje: 4 },
                            { respuesta: '2 1/2 ctvs.', puntaje: 6 },
                            { respuesta: '9 ctvs.', puntaje: 8 },
                        ],
                        respCorrecta:'B',
                    }
                },
                {
                    pregunta: {
                        pre_id: 4,
                        preguntas: [
                            { a: '¿Cuantos centimetros cubicos tendra una caja que mide 5 centimetros de largo, 4 centimetros de ancho y 3 centimetros de alto?' },
                            { b: null },
                        ],
                        resp: [
                            { respuesta: '12', puntaje: 2 },
                            { respuesta: '32', puntaje: 4 },
                            { respuesta: '60', puntaje: 6 },
                            { respuesta: '100', puntaje: 8 },
                        ],
                        respCorrecta:'C',
                    }
                },
                {
                    pregunta: {
                        pre_id: 5,
                        preguntas: [
                            { a: 'A un escolar le dicen 20 palabras de ortografia dudosa, pero solo ha escrito correctamente el 90% de ellas.' },
                            { b: '¿Cuantas palabras ha escrito correctamente?' },
                        ],
                        resp: [
                            { respuesta: '1', puntaje: 2 },
                            { respuesta: '11', puntaje: 4 },
                            { respuesta: '18', puntaje: 6 },
                            { respuesta: '19', puntaje: 8 },
                        ],
                        respCorrecta:'C',
                    }
                },
                {
                    pregunta: {
                        pre_id: 6,
                        preguntas: [
                            { a: '¿Cuantas estampillas de 1.50 Bs. podrán cambiarse por 30 estampillas de 0.5 Bs ?' },
                            { b: null },
                        ],
                        resp: [
                            { respuesta: '10', puntaje: 2 },
                            { respuesta: '15', puntaje: 4 },
                            { respuesta: '18', puntaje: 6 },
                            { respuesta: '19', puntaje: 8 },
                        ],
                        respCorrecta:'A',
                    }
                },
                {
                    pregunta: {
                        pre_id: 7,
                        preguntas: [
                            { a: 'Un equipo de futbol ha jugago 25 partidos y ganó 7 partidos más que los perdidos.' },
                            { b: '¿Cuantos partidos ha ganado?' },
                        ],
                        resp: [
                            { respuesta: '7', puntaje: 2 },
                            { respuesta: '9', puntaje: 4 },
                            { respuesta: '16', puntaje: 6 },
                            { respuesta: '18', puntaje: 8 },
                        ],
                        respCorrecta:'C',
                    }
                },
                {
                    pregunta: {
                        pre_id: 8,
                        preguntas: [
                            { a: '¿ Cuantas hojas de papel de 7x10 cm. se podrá obtener en un pliego grande de papel que mide 21x30 cm. ?' },
                            { b: null },
                        ],
                        resp: [
                            { respuesta: '3', puntaje: 2 },
                            { respuesta: '6', puntaje: 4 },
                            { respuesta: '9', puntaje: 6 },
                            { respuesta: '34', puntaje: 8 },
                        ],
                        respCorrecta:'C',
                    }
                },
                {
                    pregunta: {
                        pre_id: 9,
                        preguntas: [
                            { a: 'Si un metro de papel cuesta 10 ctvs.' },
                            { b: '¿Cuantos se tendra que gastar para cubrir el techo de un cuarto de 15 metros?' },
                        ],
                        resp: [
                            { respuesta: '3.10', puntaje: 2 },
                            { respuesta: '6.20', puntaje: 4 },
                            { respuesta: '31.00', puntaje: 6 },
                            { respuesta: '24.00', puntaje: 8 },
                        ],
                        respCorrecta:'D',
                    }
                },
                {
                    pregunta: {
                        pre_id: 10,
                        preguntas: [
                            { a: '¿Cual es el número que 2.5 veces es igual a 40?' },
                            { b: null },
                        ],
                        resp: [
                            { respuesta: '16', puntaje: 2 },
                            { respuesta: '8', puntaje: 4 },
                            { respuesta: '15', puntaje: 6 },
                            { respuesta: '17', puntaje: 8 },
                        ],
                        respCorrecta:'A',
                    }
                },
                {
                    pregunta: {
                        pre_id: 11,
                        preguntas: [
                            { a: 'Si un cubo de hielo de 5 pulgadas pesa 4.25 de onzas.' },
                            { b: '¿Cunatas onzas pesará un cubo de 10 pulgadas?' },
                        ],
                        resp: [
                            { respuesta: '212.5', puntaje: 2 },
                            { respuesta: '8.5', puntaje: 4 },
                            { respuesta: '34', puntaje: 6 },
                            { respuesta: '50', puntaje: 8 },
                        ],
                        respCorrecta:'C',
                    }
                },
                {
                    pregunta: {
                        pre_id: 12,
                        preguntas: [
                            { a: '¿Cual es el número que multiplicado por 2, es 4 unidades nemos que 3 veces 6?' },
                            { b: null },
                        ],
                        resp: [
                            { respuesta: '6', puntaje: 2 },
                            { respuesta: '7', puntaje: 4 },
                            { respuesta: '14', puntaje: 6 },
                            { respuesta: '8', puntaje: 8 },
                        ],
                        respCorrecta:'B',
                    }
                },
                {
                    pregunta: {
                        pre_id: 13,
                        preguntas: [
                            { a: 'Juan tiene una edad que equivale a 1/4 de la edad de su tio. Las edades de ambos reunidas dan un total de 40 años.' },
                            { b: '¿Cuantos años de diferencia hay entre la edad de Juan y la edad de su tio?' },
                        ],
                        resp: [
                            { respuesta: '10', puntaje: 2 },
                            { respuesta: '20', puntaje: 4 },
                            { respuesta: '24', puntaje: 6 },
                            { respuesta: '30', puntaje: 8 },
                        ],
                        respCorrecta:'C',
                    }
                },
                {
                    pregunta: {
                        pre_id: 14,
                        preguntas: [
                            { a: 'Un tanque de agua es alimento por 3 cañerias, una de las cuales puede llevarlo en 2 horas, y la otra en 3 horas. Una tercera cañeria puede desocuparlo en 1 hora.' },
                            { b: 'Si el tanque esta lleno, y las tres cañerias abiertas, ¿en cuantas horas estará vacio el tanque?' },
                        ],
                        resp: [
                            { respuesta: '2', puntaje: 2 },
                            { respuesta: '4', puntaje: 4 },
                            { respuesta: '5', puntaje: 6 },
                            { respuesta: '6', puntaje: 8 },
                        ],
                        respCorrecta:'D',
                    }
                },
            ]
        }
    },
]

export const conceptosVerBales = [
    {
        title: 'TEST 7',
        id: 8,
        factor: 'CONCEPTOS VERBALES',
        description: 'logico',
        contenido: {
            instructions: 'Lo que usted tiene que hacer es decidir, cual de las cuatro palabras escritas con la letras minusculas posee un significado igual o semejante a la palabra escrita con mayúscula.',
            ejm: {
                preguntas: 'CAMINAR',
                resp: [
                    { respuesta: 'Jugar' },
                    { respuesta: 'Luchar' },
                    { respuesta: 'Andar' },
                    { respuesta: 'Viajar' },
                ]
            },
            preguntas: [
                {
                    pregunta: {
                        pre_id: 0,
                        preguntas: 'EXTRAJERO',
                        resp: [
                            { respuesta: 'Nativo', puntaje: 2 },
                            { respuesta: 'Forastero', puntaje: 4 },
                            { respuesta: 'Raro', puntaje: 6 },
                            { respuesta: 'Desconocido', puntaje: 8 },
                        ],
                        respCorrecta:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 1,
                        preguntas:'CONTESTACIÓN',
                        resp: [
                            { respuesta: 'Respuesta', puntaje: 2 },
                            { respuesta: 'Conversación', puntaje: 4 },
                            { respuesta: 'Dictamen', puntaje: 6 },
                            { respuesta: 'Negación', puntaje: 8 },
                        ],
                        respCorrecta:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 2,
                        preguntas: 'ASISTIR',
                        resp: [
                            { respuesta: 'Consentir', puntaje: 2 },
                            { respuesta: 'Agregar', puntaje: 4 },
                            { respuesta: 'Concurrir', puntaje: 6 },
                            { respuesta: 'Vigilar', puntaje: 8 },
                        ],
                        respCorrecta:'3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 3,
                        preguntas: 'ADMIRADO',
                        resp: [
                            { respuesta: 'Admitido', puntaje: 2 },
                            { respuesta: 'Protegido', puntaje: 4 },
                            { respuesta: 'Asustado', puntaje: 6 },
                            { respuesta: 'Asombrado', puntaje: 8 },
                        ],
                        respCorrecta:'1',
                    }
                },
                {
                    pregunta: {
                        pre_id: 4,
                        preguntas: 'FINALIZAR',
                        resp: [
                            { respuesta: 'Limitar', puntaje: 2 },
                            { respuesta: 'Concluir', puntaje: 4 },
                            { respuesta: 'Fijar', puntaje: 6 },
                            { respuesta: 'Llegar', puntaje: 8 },
                        ],
                        respCorrecta:'1',
                    }
                },
                {
                    pregunta: {
                        pre_id: 5,
                        preguntas: 'ESTIMAR',
                        resp: [
                            { respuesta: 'Apreciar', puntaje: 2 },
                            { respuesta: 'Respetar', puntaje: 4 },
                            { respuesta: 'Estimular', puntaje: 6 },
                            { respuesta: 'Despreciar', puntaje: 8 },
                        ],
                        respCorrecta:'1',
                    }
                },
                {
                    pregunta: {
                        pre_id: 6,
                        preguntas: 'ADQUIRIR',
                        resp: [
                            { respuesta: 'Conducir', puntaje: 2 },
                            { respuesta: 'Consentir', puntaje: 4 },
                            { respuesta: 'Obtener', puntaje: 6 },
                            { respuesta: 'Aceptar', puntaje: 8 },
                        ],
                        respCorrecta:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 7,
                        preguntas: 'AMPLIO',
                        resp: [
                            { respuesta: 'Concurrido', puntaje: 2 },
                            { respuesta: 'Espacioso', puntaje: 4 },
                            { respuesta: 'Generoso', puntaje: 6 },
                            { respuesta: 'Vacio', puntaje: 8 },
                        ],
                        respCorrecta:'1',
                    }
                },
                {
                    pregunta: {
                        pre_id: 8,
                        preguntas: 'ASOMBRO',
                        resp: [
                            { respuesta: 'Sorpresa', puntaje: 2 },
                            { respuesta: 'Verguenza', puntaje: 4 },
                            { respuesta: 'Contrariedad', puntaje: 6 },
                            { respuesta: 'Peligro', puntaje: 8 },
                        ],
                        respCorrecta:'1',
                    }
                },
                {
                    pregunta: {
                        pre_id: 9,
                        preguntas: 'OPRIMIDO',
                        resp: [
                            { respuesta: 'Necesitado', puntaje: 2 },
                            { respuesta: 'Molesto', puntaje: 4 },
                            { respuesta: 'Arbitrario', puntaje: 6 },
                            { respuesta: 'Sometido', puntaje: 8 },
                        ],
                        respCorrecta:'3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 10,
                        preguntas: 'SAQUEAR',
                        resp: [
                            { respuesta: 'Recoger', puntaje: 2 },
                            { respuesta: 'Despojar', puntaje: 4 },
                            { respuesta: 'Pelear', puntaje: 6 },
                            { respuesta: 'Vestir', puntaje: 8 },
                        ],
                        respCorrecta:'3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 11,
                        preguntas: 'OBSTINACION',
                        resp: [
                            { respuesta: 'Obediencia', puntaje: 2 },
                            { respuesta: 'Terquedad', puntaje: 4 },
                            { respuesta: 'Rebeldia', puntaje: 6 },
                            { respuesta: 'Tino', puntaje: 8 },
                        ],
                        respCorrecta:'3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 12,
                        preguntas: 'ETERNO',
                        resp: [
                            { respuesta: 'Breve', puntaje: 2 },
                            { respuesta: 'Sagrado', puntaje: 4 },
                            { respuesta: 'Perpetuo', puntaje: 6 },
                            { respuesta: 'Durable', puntaje: 8 },
                        ],
                        respCorrecta:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 13,
                        preguntas: 'FUGAR',
                        resp: [
                            { respuesta: 'Engañar', puntaje: 2 },
                            { respuesta: 'Liberar', puntaje: 4 },
                            { respuesta: 'Raptar', puntaje: 6 },
                            { respuesta: 'Escapar', puntaje: 8 },
                        ],
                        respCorrecta:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 14,
                        preguntas: 'NOTABLE',
                        resp: [
                            { respuesta: 'Severo', puntaje: 2 },
                            { respuesta: 'Terrible', puntaje: 4 },
                            { respuesta: 'Anotado', puntaje: 6 },
                            { respuesta: 'Famoso', puntaje: 8 },
                        ],
                        respCorrecta:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 15,
                        preguntas: 'IMPLORAR',
                        resp: [
                            { respuesta: 'Pensar', puntaje: 2 },
                            { respuesta: 'Suplicar', puntaje: 4 },
                            { respuesta: 'Reparar', puntaje: 6 },
                            { respuesta: 'Protestar', puntaje: 8 },
                        ],
                        respCorrecta:'4',
                    }
                },
                {
                    pregunta: {
                        pre_id: 16,
                        preguntas: 'MENGUAR',
                        resp: [
                            { respuesta: 'Disimular', puntaje: 2 },
                            { respuesta: 'Repetir', puntaje: 4 },
                            { respuesta: 'Reducir', puntaje: 6 },
                            { respuesta: 'Aumentar', puntaje: 8 },
                        ],
                        respCorrecta:'4',
                    }
                },
                {
                    pregunta: {
                        pre_id: 17,
                        preguntas: 'PROFECIA',
                        resp: [
                            { respuesta: 'Segestion', puntaje: 2 },
                            { respuesta: 'Predicción', puntaje: 4 },
                            { respuesta: 'Herejía', puntaje: 6 },
                            { respuesta: 'Profesión', puntaje: 8 },
                        ],
                        respCorrecta:'4',
                    }
                },
                {
                    pregunta: {
                        pre_id: 18,
                        preguntas: 'CORROER',
                        resp: [
                            { respuesta: 'Carcomer', puntaje: 2 },
                            { respuesta: 'Perseguir', puntaje: 4 },
                            { respuesta: 'Comerciar', puntaje: 6 },
                            { respuesta: 'Corregir', puntaje: 8 },
                        ],
                        respCorrecta:'4',
                    }
                },
                {
                    pregunta: {
                        pre_id: 19,
                        preguntas: 'PERSISTENTE',
                        resp: [
                            { respuesta: 'Probable', puntaje: 2 },
                            { respuesta: 'Raro', puntaje: 4 },
                            { respuesta: 'Variable', puntaje: 6 },
                            { respuesta: 'Constante', puntaje: 8 },
                        ],
                        respCorrecta:'3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 20,
                        preguntas: 'INDAGAR',
                        resp: [
                            { respuesta: 'Hallar', puntaje: 2 },
                            { respuesta: 'Remover', puntaje: 4 },
                            { respuesta: 'Averiguar', puntaje: 6 },
                            { respuesta: 'Detener', puntaje: 8 },
                        ],
                        respCorrecta:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 21,
                        preguntas: 'IDONEIDAD',
                        resp: [
                            { respuesta: 'Suficiencia', puntaje: 2 },
                            { respuesta: 'Prudencia', puntaje: 4 },
                            { respuesta: 'Sabiduria', puntaje: 6 },
                            { respuesta: 'Donación', puntaje: 8 },
                        ],
                        respCorrecta:'4',
                    }
                },
                {
                    pregunta: {
                        pre_id: 22,
                        preguntas: 'DIAFANO',
                        resp: [
                            { respuesta: 'Luminoso', puntaje: 2 },
                            { respuesta: 'Limpido', puntaje: 4 },
                            { respuesta: 'Frágil', puntaje: 6 },
                            { respuesta: 'Delicado', puntaje: 8 },
                        ],
                        respCorrecta:'1',
                    }
                },
                {
                    pregunta: {
                        pre_id: 23,
                        preguntas: 'AMBIGUO',
                        resp: [
                            { respuesta: 'Ambicioso', puntaje: 2 },
                            { respuesta: 'Hipócrita', puntaje: 4 },
                            { respuesta: 'Cuidadoso', puntaje: 6 },
                            { respuesta: 'Dudoso', puntaje: 8 },
                        ],
                        respCorrecta:'3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 24,
                        preguntas: 'DILAPIDAR',
                        resp: [
                            { respuesta: 'Malgastar', puntaje: 2 },
                            { respuesta: 'Usar', puntaje: 4 },
                            { respuesta: 'Distraer', puntaje: 6 },
                            { respuesta: 'Idéntico', puntaje: 8 },
                        ],
                        respCorrecta:'3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 25,
                        preguntas: 'AFLIGIDO',
                        resp: [
                            { respuesta: 'Destruido', puntaje: 2 },
                            { respuesta: 'Apenado', puntaje: 4 },
                            { respuesta: 'Pesado', puntaje: 6 },
                            { respuesta: 'Derrotado', puntaje: 8 },
                        ],
                        respCorrecta:'1',
                    }
                },
                {
                    pregunta: {
                        pre_id: 26,
                        preguntas: 'DESTREZA',
                        resp: [
                            { respuesta: 'Ventaja', puntaje: 2 },
                            { respuesta: 'Destrozo', puntaje: 4 },
                            { respuesta: 'Trenzado', puntaje: 6 },
                            { respuesta: 'Habilidad', puntaje: 8 },
                        ],
                        respCorrecta:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 27,
                        preguntas: 'DEFINIR',
                        resp: [
                            { respuesta: 'Postergar', puntaje: 2 },
                            { respuesta: 'Difamar', puntaje: 4 },
                            { respuesta: 'Referir', puntaje: 6 },
                            { respuesta: 'Denegar', puntaje: 8 },
                        ],
                        respCorrecta:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 28,
                        preguntas: 'AVARO',
                        resp: [
                            { respuesta: 'Tacaño', puntaje: 2 },
                            { respuesta: 'Rico', puntaje: 4 },
                            { respuesta: 'Usurero', puntaje: 6 },
                            { respuesta: 'Explotador', puntaje: 8 },
                        ],
                        respCorrecta:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 29,
                        preguntas: 'DELICADO',
                        resp: [
                            { respuesta: 'Delicioso', puntaje: 2 },
                            { respuesta: 'Hermoso', puntaje: 4 },
                            { respuesta: 'Fino', puntaje: 6 },
                            { respuesta: 'Bueno', puntaje: 8 },
                        ],
                        respCorrecta:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 30,
                        preguntas: 'PRESAGIAR',
                        resp: [
                            { respuesta: 'Enfermar', puntaje: 2 },
                            { respuesta: 'Peligrar', puntaje: 4 },
                            { respuesta: 'Predestinar', puntaje: 6 },
                            { respuesta: 'Prever', puntaje: 8 },
                        ],
                        respCorrecta:'3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 31,
                        preguntas: 'CAPTURAR',
                        resp: [
                            { respuesta: 'Apresar', puntaje: 2 },
                            { respuesta: 'Amarrar', puntaje: 4 },
                            { respuesta: 'Encerrar', puntaje: 6 },
                            { respuesta: 'Castigar', puntaje: 8 },
                        ],
                        respCorrecta:'3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 32,
                        preguntas:'VITUPERIO',
                        resp: [
                            { respuesta: 'Blasfemia', puntaje: 2 },
                            { respuesta: 'Calumnia', puntaje: 4 },
                            { respuesta: 'Regocijo', puntaje: 6 },
                            { respuesta: 'Agresión', puntaje: 8 },
                        ],
                        respCorrecta:'4',
                    }
                },
                {
                    pregunta: {
                        pre_id: 33,
                        preguntas: 'MEDITAR',
                        resp: [
                            { respuesta: 'Intervenir', puntaje: 2 },
                            { respuesta: 'Escribir', puntaje: 4 },
                            { respuesta: 'Manifestar', puntaje: 6 },
                            { respuesta: 'Reflexionar', puntaje: 8 },
                        ],
                        respCorrecta:'4',
                    }
                },
                {
                    pregunta: {
                        pre_id: 34,
                        preguntas: 'DEMOLER',
                        resp: [
                            { respuesta: 'Cambiar', puntaje: 2 },
                            { respuesta: 'Mudar', puntaje: 4 },
                            { respuesta: 'Arrestar', puntaje: 6 },
                            { respuesta: 'Derribar', puntaje: 8 },
                        ],
                        respCorrecta:'3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 35,
                        preguntas: 'DISCREPAR',
                        resp: [
                            { respuesta: 'Acodar', puntaje: 2 },
                            { respuesta: 'Oponer', puntaje: 4 },
                            { respuesta: 'Enemistar', puntaje: 6 },
                            { respuesta: 'Increpar', puntaje: 8 },
                        ],
                        respCorrecta:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 36,
                        preguntas: 'FALAZ',
                        resp: [
                            { respuesta: 'Antipatico', puntaje: 2 },
                            { respuesta: 'Mordaz', puntaje: 4 },
                            { respuesta: 'Engañoso', puntaje: 6 },
                            { respuesta: 'Veraz', puntaje: 8 },
                        ],
                        respCorrecta:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 37,
                        preguntas: 'EXTEMPORANEO',
                        resp: [
                            { respuesta: 'Raro', puntaje: 2 },
                            { respuesta: 'Ambiguo', puntaje: 4 },
                            { respuesta: 'Inoportuno', puntaje: 6 },
                            { respuesta: 'Extraordinario', puntaje: 8 },
                        ],
                        respCorrecta:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 38,
                        preguntas: 'PROCAZ',
                        resp: [
                            { respuesta: 'Grosero', puntaje: 2 },
                            { respuesta: 'Educado', puntaje: 4 },
                            { respuesta: 'Eficaz', puntaje: 6 },
                            { respuesta: 'Huraño', puntaje: 8 },
                        ],
                        respCorrecta:'3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 39,
                        preguntas: 'CONCEDER',
                        resp: [
                            { respuesta: 'Conciliar', puntaje: 2 },
                            { respuesta: 'Aconcejar', puntaje: 4 },
                            { respuesta: 'Intentar', puntaje: 6 },
                            { respuesta: 'Otorgar', puntaje: 8 },
                        ],
                        respCorrecta:'1',
                    }
                },
                {
                    pregunta: {
                        pre_id: 40,
                        preguntas: 'INSOLITO',
                        resp: [
                            { respuesta: 'Solidario', puntaje: 2 },
                            { respuesta: 'Frecuente', puntaje: 4 },
                            { respuesta: 'Solitario', puntaje: 6 },
                            { respuesta: 'Desacostumbrado', puntaje: 8 },
                        ],
                        respCorrecta:'1',
                    }
                },
                {
                    pregunta: {
                        pre_id: 41,
                        preguntas: 'ANTECEDER',
                        resp: [
                            { respuesta: 'Pretender', puntaje: 2 },
                            { respuesta: 'Preceder', puntaje: 4 },
                            { respuesta: 'Seguir', puntaje: 6 },
                            { respuesta: 'Suceder', puntaje: 8 },
                        ],
                        respCorrecta:'1',
                    }
                },
                {
                    pregunta: {
                        pre_id: 42,
                        preguntas: 'INVIDENTE',
                        resp: [
                            { respuesta: 'Ciego', puntaje: 2 },
                            { respuesta: 'Evidente', puntaje: 4 },
                            { respuesta: 'Visible', puntaje: 6 },
                            { respuesta: 'Visionario', puntaje: 8 },
                        ],
                        respCorrecta:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 43,
                        preguntas: 'MENOSPRECIAR',
                        resp: [
                            { respuesta: 'Justificar', puntaje: 2 },
                            { respuesta: 'Descuidar', puntaje: 4 },
                            { respuesta: 'Despreciar', puntaje: 6 },
                            { respuesta: 'Desperdiciar', puntaje: 8 },
                        ],
                        respCorrecta:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 44,
                        preguntas: 'ADVERSIDAD',
                        resp: [
                            { respuesta: 'Veracidad', puntaje: 2 },
                            { respuesta: 'Venganza', puntaje: 4 },
                            { respuesta: 'Advertencia', puntaje: 6 },
                            { respuesta: 'Infortunio', puntaje: 8 },
                        ],
                        respCorrecta:'1',
                    }
                },
                {
                    pregunta: {
                        pre_id: 45,
                        preguntas: 'DEPARTIR',
                        resp: [
                            { respuesta: 'Conversar', puntaje: 2 },
                            { respuesta: 'Dividir', puntaje: 4 },
                            { respuesta: 'Viajar', puntaje: 6 },
                            { respuesta: 'Deponer', puntaje: 8 },
                        ],
                        respCorrecta:'4',
                    }
                },
                {
                    pregunta: {
                        pre_id: 46,
                        preguntas: 'PROMOVER',
                        resp: [
                            { respuesta: 'Abrigar', puntaje: 2 },
                            { respuesta: 'Remover', puntaje: 4 },
                            { respuesta: 'Envolver', puntaje: 6 },
                            { respuesta: 'Fomentar', puntaje: 8 },
                        ],
                        respCorrecta:'4',
                    }
                },
                {
                    pregunta: {
                        pre_id: 47,
                        preguntas: 'SOLIVIANTAR',
                        resp: [
                            { respuesta: 'Inquietar', puntaje: 2 },
                            { respuesta: 'Sofocar', puntaje: 4 },
                            { respuesta: 'Elevar', puntaje: 6 },
                            { respuesta: 'Gritar', puntaje: 8 },
                        ],
                        respCorrecta:'2',
                    }
                },
                {
                    pregunta: {
                        pre_id: 48,
                        preguntas: 'TRANSCRIBIR',
                        resp: [
                            { respuesta: 'Dibujar', puntaje: 2 },
                            { respuesta: 'Copiar', puntaje: 4 },
                            { respuesta: 'Describir', puntaje: 6 },
                            { respuesta: 'Traspasar', puntaje: 8 },
                        ],
                        respCorrecta:'3',
                    }
                },
                {
                    pregunta: {
                        pre_id: 49,
                        preguntas: 'COHESION',
                        resp: [
                            { respuesta: 'Choque', puntaje: 2 },
                            { respuesta: 'Unión', puntaje: 4 },
                            { respuesta: 'Prohibición', puntaje: 6 },
                            { respuesta: 'Injuria', puntaje: 8 },
                        ],
                        respCorrecta:'1',
                    }
                },
            ]
        }
    }
]