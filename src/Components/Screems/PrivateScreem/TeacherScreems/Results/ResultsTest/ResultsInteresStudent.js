...
const ResultsInteresStudent = ({ navigation, route }) => {
  ...
  for (var i = 0; i < result.length; i++) {
    var sum = parseInt(result[i].pregunta1) + parseInt(result[i].pregunta2) + parseInt(result[i].pregunta3) + parseInt(result[i].pregunta4) + parseInt(result[i].pregunta5) + parseInt(result[i].pregunta6)
    array.push({
      totalSeccion: sum,
      ...
      <ScrollView>
        {array.length > 0 ? (
          <>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-evenly', backgroundColor: '#12151C', padding: 5, margin: 5 }}>
              <Text style={{ color: 'white' }}>AIRE LIBRE</Text>
...
export default ResultsInteresStudent